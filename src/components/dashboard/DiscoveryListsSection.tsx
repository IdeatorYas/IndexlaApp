...
function DiscoveryColumn({
  ...
  return (
    <div className={`app-panel overflow-hidden ${className}`}>
      <div className="flex items-center justify-between gap-2 border-b border-app-line px-2 py-1">
        <DashboardSectionHeading label={title} tone={tone} size="sm" as="h3" />
        <Link
          href={APP_ROUTES.discover}
          className="text-[8px] font-bold text-app-brand hover:underline"
        >
          View All →
        </Link>
      </div>
      {products.length === 0 ? (
        <p className="px-2 py-1.5 text-[9px] text-app-dim">No products yet.</p>
      ) : (
...
function DiscoveryRow({ product }: { product: MarketplaceProductPreview }) {
  ...
  return (
    <Link
      href={product.href}
      className="flex items-center gap-1.5 px-2 py-1 transition-colors hover:bg-app-panel"
    >
      <AssetIconStack assetIds={product.assetIds} size={16} max={3} />
      <div className="min-w-0 flex-1">
        <p className="truncate text-[10px] font-bold leading-tight text-app-ink">
          {product.name}
        </p>
        <p className="truncate text-[8px] leading-tight text-app-dim">
          {product.creatorName} · {formatUsd(product.aumUsd, true)}
        </p>
      </div>
      <p
        className={[
          "app-metric shrink-0 text-[10px] font-bold",
