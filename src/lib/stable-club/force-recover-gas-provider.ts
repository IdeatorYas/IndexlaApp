 ...
 * Inflate eth_estimateGas + force send/sign gas for Uni recover approve/swap/sweep.
 */
import {
  forceRecoverTxGas,
  inflateRecoverEstimateGasHex,
  recoverGasFloorForCall,
...
export function wrapProviderForceRecoverGas<T>(provider: T): T {
  ...
  const request: RequestFn = async (args) => {
    ...
    if (method === "eth_estimateGas" && Array.isArray(params) && params[0]) {
      ...
      const floor = recoverGasFloorForCall({ to, data });
      const result = await baseRequest({ method, params });
      if (floor != null && typeof result === "string") {
        return inflateRecoverEstimateGasHex(result, floor);
      }
      return result;
    }
